...
当前仓库已实现完整 LocalSend v2.1 收发链路和客户端式 Web 管理界面：可在桌面或移动浏览器中查看节点状态、附近/信任设备、接收审批、发送进度和文件级双向传输历史，并通过多级目录弹层选择文件或整个目录。发送开始后会切换到全屏设备流向界面，展示发送端、接收端、等待响应或实时进度，并可直接取消。历史记录支持文件信息、打开文件、单条删除和清空；删除记录不会删除实体文件。业务状态使用 SSE 长连接实时推送，不依赖浏览器定时轮询。
